...
def makeMakefile(f,url):
	...
	f.write("build/"+url.lstrip("./")+".link: "+url.lstrip("./")+" "+" ".join(map(lambda a:sep.join(["build"+base[3:].lstrip("./"),a.lstrip("./")])+".url",deps))+"\n")
	f.write("\t@test -d "+(" build/"+base[4:].lstrip("./"))+" || mkdir -p "+(" build/"+base[4:].lstrip("./"))+"\n")
